			...
			for(i=0; i<nx; i++ ) {
				...
				for (k=0; k<nheight; k++) {
					if (temphand <= h[k]){
						...
						SurfaceArea[tempcatch][k] = SurfaceArea[tempcatch][k] + dxA*dyA;
						BedArea[tempcatch][k] = BedArea[tempcatch][k] + dxA*dyA*sqrt(1 + tempslp*tempslp);
						Volume[tempcatch][k] = Volume[tempcatch][k] + (h[k] - temphand)*dxA*dyA;
					}
				}
			}
 		}
