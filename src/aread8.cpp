...
int aread8(char *pfile, char *afile, char *datasrc, char *lyrname, int uselyrname, int lyrno, char *wfile, int useOutlets, int usew, int contcheck) {
    MPI_Init(NULL,NULL);
    {
        ...
        // Check file existence on rank 0 only
        if (rank == 0) {
        ...
        // Open the dataset with error handling
        GDALDatasetH hDataset = GDALOpen(pfile, GA_ReadOnly);
        if (hDataset == NULL) {
            if (rank == 0) {
        ...
        if (proj == NULL) {
            if (rank == 0) {
                fprintf(stderr, "Warning: No projection information found in %s\n", pfile);
            }
        }
