...
int aread8( char* pfile, char* afile, char *datasrc,int lyrno, char *wfile, int useOutlets, int usew, int contcheck) {
	...
	if( useOutlets == 1) {
		if(rank==0){
			if(readoutlets(datasrc,lyrno,hSRSRaster, &numOutlets, x, y)==0){
				usingShapeFile=true;
